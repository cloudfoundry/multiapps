...
public class ActivitiRetryAction extends AbstractStepRelatedAction {
    ...
    private void resetTaskCounter() {
        boolean taskHasTryCount = getDefaultProcessEngine().getRuntimeService()
            .hasVariable(getProcessInstanceId(), constructLogicalStepContextVariable(TRYCOUNT_CONTEXT_PREFIX));

        if (taskHasTryCount) {
            getDefaultProcessEngine().getRuntimeService()
                .setVariable(getProcessInstanceId(), constructLogicalStepContextVariable(TRYCOUNT_CONTEXT_PREFIX), RESET_RETRY_COUNT_VALUE);
        }
    }

    private void setStatusToFailed() {
        getDefaultProcessEngine().getRuntimeService()
            .setVariable(getProcessInstanceId(), constructLogicalStepContextVariable(Constants.STEP_NAME_PREFIX),
