...
public class CompoundGrouper<T> {
    ...
    public Map<List<Entry<String, String>>, List<T>> group(List<T> allInstances) {
        ...
        for (IGrouper<T> grouper : groupers) {
            ...
            for (Entry<List<Entry<String, String>>, List<T>> groupEntry : groupedInstances.entrySet()) {

                Map<String, List<T>> newGroup = grouper.doGroup(groupEntry.getValue());

                List<Entry<String, String>> oldKey = groupEntry.getKey();

                for (Entry<String, List<T>> nextGroup : newGroup.entrySet()) {
                    List<Entry<String, String>> newKey = new ArrayList<Entry<String, String>>(oldKey);
                    newKey.add(new SimpleEntry<String, String>(grouper.getGroupName(), nextGroup.getKey()));
