...
public class LoggerTest {
    ...
    public void init() {
        ...
        Mockito.when(loggerImpl.isDebugEnabled()).thenReturn(true);
        ...
        Mockito.when(context.getEngineServices()).thenReturn(engineServices);
        Mockito.when(context.getProcessDefinitionId()).thenReturn("PD_ID");
        Mockito.when(context.getProcessInstanceId()).thenReturn("PI_ID");
        Mockito.when(context.getCurrentActivityId()).thenReturn("CA_ID");
    }

    @Test
    public void testDebugWithNullContext() {
        logger.debug(null, "Hello {}", "world");
        Mockito.verify(loggerImpl).debug("[{}][{}][{}] Hello {}", null, null, null, "world");
    }

    @Test
    public void testDebugWithoutReplacements() {
        logger.debug(context, "Hello world");
        Mockito.verify(loggerImpl).debug("[{}][{}][{}] Hello world", "PD_ID", "PI_ID", "CA_ID");
    }

    @Test
    public void testDebugWithOneReplacement() {
        logger.debug(context, "Hello world {}", "again");
        Mockito.verify(loggerImpl).debug("[{}][{}][{}] Hello world {}", "PD_ID", "PI_ID", "CA_ID", "again");
    }

    @Test
    public void testDebugWithMultipleReplacement() {
        logger.debug(context, "Hello world {}. {}+{} is {}", "again", 1, "1", 2);
        Mockito.verify(loggerImpl).debug("[{}][{}][{}] Hello world {}. {}+{} is {}", "PD_ID", "PI_ID", "CA_ID", "again", 1, "1", 2);
    }
    ...
    public void testErrorWithThrowable() {
        ...
        Mockito.verify(loggerImpl).error("[{}][{}][{}] Execution failed", "PD_ID", "PI_ID", "CA_ID", e);
    }
    ...
    public void testErrorWithReplacements() {
        ...
        Mockito.verify(loggerImpl).error("[{}][{}][{}] Execution failed for methods {} and {}", "PD_ID", "PI_ID", "CA_ID", "one", "two", e);
    }

    @Test
    public void testWhenNotInDebugMode() {
        Mockito.when(loggerImpl.isDebugEnabled()).thenReturn(false);
        logger.debug(context, "This shold {} be logged", "NOT");
        Mockito.verify(loggerImpl).isDebugEnabled();
