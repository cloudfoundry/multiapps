...
import com.sap.activiti.common.impl.AbstractActivitiStepWithBridge;
...
public abstract class AbstractSLProcessStepWithBridge extends AbstractActivitiStepWithBridge implements StepIndexProvider {
    ...
    protected void preExecuteStep(DelegateExecution context) throws SLException {
        getStepHelper().preExecuteStep(context, getStatus(context));
    }
    ...
    private ExecutionStatus getStatus(DelegateExecution context) {
        Object status = context.getVariable(getStatusVariable());
        return (status == null) ? ExecutionStatus.NEW : ExecutionStatus.valueOf((String) status);
    }

    @Override
    public Integer getDefaultStepIndex(DelegateExecution context) {
        return (int) context.getVariable(getIndexVariable());
    }

    protected abstract String getIndexVariable();
